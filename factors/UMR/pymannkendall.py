...
def __mk_score_up(x, n):
    s_up = 0

    demo = np.ones(n) 
    for k in range(n-1):
        s_up = s_up + np.sum(demo[k+1:n][x[k+1:n] > x[k]])
...
def __variance_s(x, n):
    ...
    # 若只能分一组，即全部样本值一致，则var_s最小，为0； 若可以分n组，则var_s最大
    return var_s
...
def sens_estimator(x):
    idx = 0
    n = len(x)
    d = np.ones(int(n*(n-1)/2))# n*(n-1)/2为1~n求和，即创建长度为样本量一半的数组，即一个移动、同向、不重合的组合次数

    for i in range(n-1):# 位置列表1
        j = np.arange(i+1,n)# 位置列表2
        # idx位置，至往后len(j)个位置上的斜率
        d[idx : idx + len(j)] = (x[j] - x[i]) / (j - i)
        idx = idx + len(j)
        # slope：每一单位序列差距 的 值的变化率
    return d
...
#     x, n = __missing_values_analysis(x, method = 'skip')
    ...
    # intercept 越小 则  中间位置 * 单位位置变化量 越接近于 序列的中位数 
